...
function tve_compat_content_filters_before_shortcode( $content ) {
	 ...
	 */
	if ( function_exists( 'pmpro_wp' ) && ! function_exists( 'pmpro_pages_shortcode' ) ) {
		global $post;
...
function tve_compat_content_filters_after_shortcode( $content ) {
	...
	/**
	 * Call Mail Poet scan_content function to replace [wysija_page] placeholder
	 * with its form
	 */
	if ( class_exists( 'WYSIJA_help_front' ) ) {
		$wysija_help_front = new WYSIJA_help_front();
		$content           = $wysija_help_front->scan_content( $content );
	}
...
if ( function_exists( 'pmpro_wp' ) ) {
	function tve_pmpro_shortcodes() {
		global $post;
		if ( ! empty( $post ) ) {
			$tve_content                  = tve_get_post_meta( $post->ID, 'tve_save_post' );
