	...
	<?php if ( ! empty( $selected_api ) ) : ?>
		...
		<div class="tve-list-container tve-api-option-group tve-api-option-group-list" <?php echo $api_key == 'drip' && isset( $extra_settings[ $api_key ]['type'] ) && $extra_settings[ $api_key ]['type'] == 'automation' ? 'style="display:none"' : ''; ?>>
			...
			<?php else : ?>
				...
				<a href="javascript:void(0)" class="tve_click tve_lightbox_link tve_lightbox_link_refresh" data-ctrl="function:auto_responder.api.reload_lists"
				   data-force-fetch="1"" data-api="<?php echo $api_key ?>"><?php echo __( 'Reload', TVE_DASH_TRANSLATE_DOMAIN ) ?></a>
				<?php
